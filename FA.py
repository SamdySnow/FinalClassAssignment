...
def init(item_weight,item_value,pack_weight=700,xSize_=200,iteration_=1000,c1_=0.5,c2_=0.5,w_=0.8):
    ...
    for i in pi:
        pi_tag.append(i[1])
    #按照价值重量比从大到小，对所有个体进行优化，对已经加入的物品来说，若加入重量不超则加入，若超出背包重量则取出
...
def solve(we,va,pw,T=100,i=None):   
    #寻找粒子群最优位置和单个粒子
    global x,fgbest,v,Dim
    ...
    # if 1<=i<20:
    now_price = calc_price(gbest,va)
    now = copy.deepcopy(gbest)
    for i in range(xSize):
        r = np.random.randint(0,Dim,(1,))
        if xbest[i][r] == 0:
            #若物品不在背包中
            xbest[i][r] = 1  # 直接放入背包
            ...
            while(1): #并放入另一件物品
                ...
                if(xbest[i][ob] == 0):
                    ...
                    break
        #计算合法性
        if calc_weight(xbest[i],we) > pw: #超重
            continue #直接丢弃
        else:
            if calc_price(xbest[i],va) > now_price: #优于当前解
                now = copy.deepcopy(xbest[i]) #直接接受新解
                if calc_price(gbest,va) < calc_price(xbest[i],va):
                    gbest = copy.deepcopy(fxbest[i]) #更新全局最优解
            else: #劣于当前解
                g = 1.0*(calc_price(xbest[i],va) - now_price)/(T+1e-20)
                if(random.random() < np.exp(g)):  # 概率接受劣解
                    now = copy.deepcopy(xbest[i])
        xbest[i] = copy.deepcopy(now)
...
def pso(item_weight,item_value,pack_weight,xSize_=200,iteration_=1000,c1_=0.5,c2_=0.5,w_=0.8):
    """
    ...
    -------
    """
    init(item_weight,item_value,pack_weight,xSize_,iteration_,c1_,c2_,w_)
    fgbest_list = []
    for i in range(iteration_):
        a = solve(item_weight,item_value,pack_weight,i)
